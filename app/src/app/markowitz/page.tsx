...
const fetchRiskFreeRate = async () => parseFloat(((await fetch(`${env.APP_URL}/api/utils/risk-free-rate`).then((r) => r.json())) as number).toFixed(6))
  ...
  try {
    ...
    return (
      <Flex direction='column' gap='6'>
        <Card className='!p-4' style={{ background: 'linear-gradient(hsl(260deg 4.23% 9.0%), transparent)' }}>
    ...
    return (
      <Flex direction='column' justify='center'>
        <Text color='red'>
          ...
          <RadixLink asChild>
            <Link href={`?` + new URLSearchParams(searchParams as unknown as string).toString()}>try again</Link>
          </RadixLink>
